function [x,exitflag] = milp_reynicost_niacs(J, K, X, cond_prob, prob_act, joint_prob, state_prob, beta)
...
A = sparse(J*(4*K*K-J-K)+1, X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2+2 + 1); % deviation term (+1)
rowcnt = 1;
%Aeq and beq reserved for Aeqx = beq
Aeq = sparse(J*K, X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2+2 + 1); %deviation term (+1)
rowcnteq = 1;
    ...
    for l=1:J
        for k=1:K
            if(j ~= l)
                ...
                A(rowcnt,X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2+2 + 1) = -1;
...
lb = zeros(X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2+2+1,1);
...
ub(X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2+2 + 1) = Inf;
%ub(1:(X*J*K)) = 100;
f = zeros(X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2+2+1,1); % no objective function
f(X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2+2+1)=1;
