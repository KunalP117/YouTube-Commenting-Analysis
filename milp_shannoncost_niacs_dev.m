function [x,exitflag] = milp_shannoncost_niacs(J, K, X, cond_prob, prob_act)
%MILP_GENERALCOST Summary of this function goes here
%Rational inattention test

M = 1000;
%A and b are reserved for Ax <= b
A = sparse(J*(4*K*K-J-K)+1, X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2 + 2+1);
rowcnt = 1;
%Aeq and beq reserved for Aeqx = beq
Aeq = sparse(J*K, X*J*K+2*J(K-1)+2*(J^2)*(K-1)+2 + 2+1);
rowcnteq = 1;
